...
namespace Oxide.Plugins
{
	...
	internal class HighWallBarricades : RustPlugin
	{
		...
		private PluginSettings GetDefaultConfig() => new PluginSettings {
			DecayTime = 600,
			DecayDamage = 0.2f,
			InitialHealth = 1.0f,
		...
		protected override void LoadConfig()
		{
			...
			if(Settings.ConfigVersion < Version)
			{
				...
				{
					PluginSettings tempconfig = new PluginSettings {
		...
		{
			foreach(DecayEntity dEntity in UnityEngine.Object.FindObjectsOfType<DecayEntity>())
				InitEntity(dEntity.gameObject);
		}
		...
		{
			DecayEntity dEntity = gameObject?.GetComponent<DecayEntity>();
			if(dEntity != null
		...
		{
			DecayBarricade comp = entity?.GetComponent<DecayBarricade>();
		...
		private void Unload()
		{
			foreach(DecayBarricade comp in ActiveComponents)
			{
				comp.RemoveComponent();
			}
		...
		private class DecayBarricade : MonoBehaviour
		{
			...
			{
				if(DecayPool.Count > 0)
					ServerMgr.Instance.StartCoroutine(DecayCycle());
			}
