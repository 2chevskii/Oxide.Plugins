...
namespace Oxide.Plugins
{
	...
	internal class AutoFPSLimiter : CovalencePlugin
	{
		...
		{
			if(log)
				Puts("Server is empty, FPS limit set to " + EmptyFPS);
		...
		{
			if(log)
				Puts("Server is no longer empty, FPS limit set to " + NonEmptyFPS);
