...
namespace Oxide.Plugins
{
	...
	internal class Skins : RustPlugin
	{
		...
		private void CcmdAddSkin(ConsoleSystem.Arg arg)
		{
			...
			if(args.Length == 2)
			{
				...
				ulong.TryParse(args.ElementAtOrDefault(1), out ulong skinId);
		...
		private void CcmdWorkshopLoad(ConsoleSystem.Arg arg)
		{
			...
			webrequest.Enqueue("http://s3.amazonaws.com/s3.playrust.com/icons/inventory/rust/schema.json", null, (code, response) =>
			{
				...
				{
					Rust.Workshop.ItemSchema schema = JsonConvert.DeserializeObject<Rust.Workshop.ItemSchema>(response);
					Rust.Workshop.ItemSchema.Item[] items = schema.items;
					int added_count = 0;
