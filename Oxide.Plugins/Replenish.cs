...
namespace Oxide.Plugins
{
	...
	internal class Replenish : RustPlugin
	{
		private List<ReplenishableContainer> containers = new List<ReplenishableContainer>();
		private List<ReplenishPlayer> playersUsing = new List<ReplenishPlayer>();
		private Dictionary<string, string> allowableContainers = new Dictionary<string, string>();
		private StoredData data;
		private bool RequireAllSlotsEmpty;
		private int DefaultTimerLength;
		private bool UsePermissionsOnly;
		private const string adminPerm = "replenish.admin";
		private const string canEdit = "replenish.edit";
		private const string canTest = "replenish.test";
		private const string canList = "replenish.list";
		...
		private void CreateReplenishableContainer(BasePlayer player, ReplenishPlayer rPlayer, StorageContainer container, HitInfo info, string type)
		{
			...
			Vector3 worldPos = info.HitEntity.transform.position;
		...
		private void RemoveReplenishableContainer(BasePlayer player, StorageContainer container)
		{
			if(GetReplenishableContainer(container.inventory) == null)
			{
		...
		private void TestReplenishableContainer(BasePlayer player, StorageContainer container)
		{
			if(GetReplenishableContainer(container.inventory) == null)
			{
				ReplyPlayer(player, "NotReplenishing");
		...
		private void chatCmdReplenish(BasePlayer player, string cmd, string[] args)
		{
		...
		private void chatCmdReplenishAdd(BasePlayer player, string cmd, string[] args)
		{
		...
		private void chatCmdReplenishAddm(BasePlayer player, string cmd, string[] args)
		{
			int timer = DefaultTimerLength;
			if(!IsAllowed(player, canEdit))
			{
		...
		private void Add(BasePlayer player, int timer, bool multi)
		{
		...
		private void chatCmdReplenishRemove(BasePlayer player, string cmd, string[] args)
		{
			if(!IsAllowed(player, canEdit))
			{
				ReplyPlayer(player, "NoPermission");
		...
		private void chatCmdReplenishRemovem(BasePlayer player, string cmd, string[] args)
		{
			if(!IsAllowed(player, canEdit))
			{
				ReplyPlayer(player, "NoPermission");
		...
		private void Remove(BasePlayer player, bool multi)
		{
			ReplenishPlayer rPlayer;
			if(IsPlayerEditing(player))
			{
		...
		private void RemoveByID(BasePlayer player, string id)
		{
		...
		private void chatCmdReplenishTest(BasePlayer player, string cmd, string[] args)
		{
		...
		private void chatCmdReplenishTestm(BasePlayer player, string cmd, string[] args)
		{
			if(!IsAllowed(player, canTest))
			{
				ReplyPlayer(player, "NoPermission");
		...
		private void Test(BasePlayer player, bool multi)
		{
			ReplenishPlayer rPlayer;
			if(IsPlayerEditing(player))
			{
		...
		private void chatCmdReplenishList(BasePlayer player, string cmd, string[] args)
		{
		...
		private void List(BasePlayer player)
		{
		...
		private void chatCmdReplenishStop(BasePlayer player, string cmd, string[] args)
		{
			if(!IsAllowed(player, canEdit))
			{
				ReplyPlayer(player, "NoPermission");
		...
		private bool IsPlayerEditing(BasePlayer player)
		{
		...
		private bool IsPlayerAdding(BasePlayer player)
		{
			foreach(ReplenishPlayer rPlayer in playersUsing)
			{
				if(rPlayer.player == player)
		...
		private bool IsPlayerRemoving(BasePlayer player)
		{
			foreach(ReplenishPlayer rPlayer in playersUsing)
			{
				if(rPlayer.player == player)
		...
		private bool IsPlayerTesting(BasePlayer player)
		{
			foreach(ReplenishPlayer rPlayer in playersUsing)
			{
				if(rPlayer.player == player)
		...
		private bool IsPlayerMulti(BasePlayer player)
		{
			foreach(ReplenishPlayer rPlayer in playersUsing)
			{
				if(rPlayer.player == player)
		...
		private void ReplyPlayer(BasePlayer player, string langKey) => SendReply(player, Lang("Prefix") + ": " + Lang(langKey));

		private void ReplyFormatted(BasePlayer player, string msg) => SendReply(player, Lang("Prefix") + ": " + msg);

		private ReplenishPlayer GetReplenishPlayer(BasePlayer player)
		{
			foreach(ReplenishPlayer rPlayer in playersUsing)
			{
				if(rPlayer.player = player)
		...
		private ReplenishableContainer GetReplenishableContainer(ItemContainer container)
		{
		...
		{
			if(player.IsAdmin && !UsePermissionsOnly)
				return true;
		...
		public class ReplenishPlayer
		{
			...
			public ReplenishPlayer(BasePlayer player) => this.player = player;
		...
		public class ReplenishableContainer
		{
			...
			private IEnumerable<ContainerItem> GetItems(ItemContainer container) => container.itemList.Select(item => new ContainerItem {
