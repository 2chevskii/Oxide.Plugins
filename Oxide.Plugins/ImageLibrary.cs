...
namespace Oxide.Plugins
{
	...
	internal class ImageLibrary : RustPlugin
	{
		#region Fields
					...
					foreach(Rust.Workshop.ItemSchema.Item item in items)
					{
						if(!string.IsNullOrEmpty(item.itemshortname) && !string.IsNullOrEmpty(item.icon_url))
						{
							...
							if(skinCount == 0)
								...
								identifier = $"{item.itemshortname}_{item.itemdefid}";
		...
		public List<ulong> GetImageList(string name)
		{
			...
			for(int i = 0; i < matches.Length; i++)
			{
				...
				{
					if(ulong.TryParse(matches[i].Substring(index + 1), out ulong skinID))
		...
		private class ImageAssets : MonoBehaviour
		{
			...
			private void OnDestroy() => queueList.Clear();

			public void Add(string name, string url = null, byte[] bytes = null) => queueList.Enqueue(new QueueItem(name, url, bytes));
			...
			private void Next()
			{
				...
				if(!string.IsNullOrEmpty(queueItem.url))
					...
					StoreByteArray(queueItem.bytes, queueItem.name);
			}
