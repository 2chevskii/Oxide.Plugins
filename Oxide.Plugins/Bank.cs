...
{
	[Info("Bank", "Calytic", "1.0.4")]
	[Description("Safe player storage")]
	internal class Bank : RustPlugin
	{
		#region Variables

		private string defaultBoxPrefab;
		...
		private List<string> npcids = new List<string>();
		private float radiationMax;
		private bool allowSafeZone;
		...
		public class BankProfile
		{
			...
			public BasePlayer Player {
				get => BasePlayer.Find(playerID.ToString());
				protected set => playerID = value.userID;
			}
			...
			public bool Add(List<Item> items) => Add(items.ToArray());
			...
			public bool Remove(List<Item> items) => Remove(items.ToArray());
			...
			private void PopulateContainer(BasePlayer player, ItemContainer container, List<ItemProfile> items = null)
			{
				...
				foreach(ItemProfile profile in items)
				{
					...
					if(item is Item)
					{
						...
						BaseProjectile weapon = held as BaseProjectile;
						if(weapon != null)
						{
							BaseProjectile projectile = weapon as BaseProjectile;
		...
		private void Init()
		{
			...
			datafile = new DataFileSystem(Interface.Oxide.DataDirectory + "\\" + GetConfig<string>("subDirectory", "banks"));
		...
		private new void LoadDefaultMessages() => lang.RegisterMessages(new Dictionary<string, string>
			{
		...
		private Dictionary<string, object> GetDefaultBoxes() => new Dictionary<string, object>() {
		...
		private Dictionary<string, object> GetDefaultSlots() => new Dictionary<string, object>() {
				{"bank.default", 4},
				{"bank.big", 30}
			};

		private Dictionary<string, object> GetDefaultItemLimits() => new Dictionary<string, object>() {
		...
		private void Unloaded()
		{
			foreach(BasePlayer player in BasePlayer.activePlayerList)
			{
				if(onlinePlayers.TryGetValue(player, out OnlinePlayer onlinePlayer) && onlinePlayer.View != null)
				{
		...
		private void SaveProfileByUser(ulong userID)
		{
		...
		private void SaveProfile(ulong playerID, BankProfile profile = null)
		{
			if(profile == null)
			{
				if(!banks.ContainsKey(playerID))
		...
		private bool TryGetPlayer(BaseNetworkable entity, out BasePlayer player)
		{
			player = null;
			foreach(KeyValuePair<BasePlayer, OnlinePlayer> kvp in onlinePlayers)
			{
		...
		private void AddNpc(string id) => npcids.Add(id);

		private void AddNpc(ulong id) => AddNpc(id.ToString());
		...
		private void RemoveNpc(ulong id) => RemoveNpc(id.ToString());
		...
		{
			if(entity == null || target == null || entity == target || entity.net == null)
				return null;
			if(target.IsAdmin)
				return null;

			bool IsMyBank = false;
			if(onlinePlayers.TryGetValue(target, out OnlinePlayer onlinePlayer))
			{
		...
		{
			if(TryGetPlayer(entity, out BasePlayer player))
			{
#if DEBUG
		...
		private object CanUseLockedEntity(BasePlayer player, BaseLock lockItem)
		{
			if(player == null)
			{
				return null;
			...
			if(MasterKey != null)
			{
				object result = MasterKey.Call("CanUseLockedEntity", player, lockItem);
				if(result is bool)
				{
					return null;
				}
			...
			if(lockItem is KeyLock && banks.TryGetValue(player.userID, out BankProfile bank) && bank.items != null)
			{
		...
		private void ViewBank(BasePlayer player, string command, string[] args)
		{
			if(!IsAllowed(player))
				return;
			...
			string name = args[0];
			OnlinePlayer ply = onlinePlayers[player];
			if(name == "list")
			{
				if(ply.Matches == null)
				{
			...
			List<BasePlayer> matches = FindPlayersByName(name);
			if(matches.Count < 1)
			{
				return;
			}
		...
		[ConsoleCommand("bank")]
		private void ccBank(ConsoleSystem.Arg arg) => cmdBank(arg.Connection.player as BasePlayer, arg.cmd.Name, arg.Args);

		[ChatCommand("bank")]
		private void cmdBank(BasePlayer player, string command, string[] args)
		{
			if(npconly)
				return;
		...
		private ItemLimit GetItemLimit(Item item)
		{
			if(itemLimits.TryGetValue(item.info.shortname, out object limit) && limit is JObject)
			{
		...
		private bool CanPlayerBank(BasePlayer player)
		{
			if(player.IsAdmin)
			{
				return true;
			...
			object canBank = Interface.Call("CanBank", player);
			if(canBank != null)
			{
		...
		private void ShowBank(BasePlayer player, BaseEntity target)
		{
		...
		private void HideBank(BasePlayer player)
		{
			player.EndLooting();
			OnlinePlayer ply = onlinePlayers[player];
			if(ply.View == null)
		...
		private string GetBox(BasePlayer player)
		{
			foreach(KeyValuePair<string, object> kvp in boxPrefabs)
			{
		...
		private int GetSlots(BasePlayer player)
		{
		...
		private void OpenBank(BasePlayer player, BaseEntity targArg)
		{
			...
			StorageContainer view = GameManager.server.CreateEntity(box, pos) as StorageContainer;
		...
		private void CloseBank(BasePlayer player, StorageContainer view)
		{
		...
		private void InvalidateBank(BasePlayer player, BankProfile profile, StorageContainer view)
		{
			bool returned = false;
		...
		private List<BasePlayer> FindPlayersByName(string name)
		{
			...
			foreach(BasePlayer ply in BasePlayer.sleepingPlayerList)
			{
				if(ply.displayName.ToLower().Contains(name.ToLower()))
					matches.Add(ply);
			}
		...
		private void ShowMatchingPlayers(BasePlayer player)
		{
			int i = 0;
			foreach(BasePlayer ply in onlinePlayers[player].Matches)
			{
				i++;
