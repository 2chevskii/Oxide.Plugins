...
>>>>>>> master
		{
			...
			if(!Cooldowns.ContainsKey(player.userID))
				Cooldowns.Add(player.userID, -1f);
			...
			else if(!Physics.Raycast(player.eyes.HeadRay(), out hitPoint, maxDistance: 20f))
			...
			{
				BaseEntity horse = GameManager.server.CreateEntity("assets/rust.ai/nextai/testridablehorse.prefab", hitPoint.point);
				if(horse)
				{
					horse.Spawn();
